...
advisories are closed, two of them HIGH, and one had been quietly failing
container builds on pull requests that never touched the service it broke.
Nothing in the application itself changed: outside `CHANGELOG.md`, this
release moves lockfiles, manifests, one workflow pin, and prose.

### Security

- **`transformers` 5.5.0 -> 5.15.1** (#528). CVE-2026-9856 (HIGH) --
  `save_pretrained` path traversal allowing arbitrary file writes through chat
  template names. This advisory was also why pull requests touching only
  `frontend/package-lock.json` showed red `container-build (ingest)` checks:
  Trivy gates that image on HIGH findings, so every branch cut from `main`
  inherited the failure regardless of its own diff. Worth knowing because
  `gh run view --log-failed` shows only cleanup output for it -- the findings
  table appears in the full log.
- **`browserslist` 4.28.2 -> 4.28.8** (#528). CVE-2026-73088 (HIGH) --
  uncaught crash and prototype write via untrusted `browserslist-stats.json`
  in `normalizeStats` -- and GHSA-c83g-rgw3-j3cx, unbounded memory growth from
  a cache that never evicts. The alert listed only the first; `npm audit`
  reported both.
- **`@humanfs/node` 0.16.7 -> 0.16.8** (#528). GHSA-p498-v437-472g (MEDIUM) --
  recursive copy follows symlinked files and copies data from outside the
  source tree.

All three were transitive. No manifest pinned any of them, so the fix is a
lockfile regeneration in every case.

### Dependencies

- **Eight Dependabot pull requests rolled up across five ecosystems** (#538):
  `protobuf` 7.36.1, `pydantic` 2.13.5 (with `pydantic-core` 2.46.5),
  `cachetools` 7.1.8, `poetry` 2.4.2, `sentence-transformers` 6.0.1, `hyper`
  1.11.1, `tower-http` 0.7.1, and nine frontend packages -- `@opentelemetry/*`
  to 2.11.0 / 0.222.0, `lucide-react` 1.39, `zod` 4.5.4, `globals` 17.12,
  `typescript-eslint` 8.69.
- **A `grpc` group bump does not always mean regenerated stubs** (#538). That
  group exists because `grpcio-tools` generates the committed stubs under
  `ai_resume_api/proto/`, which carry import-time version guards. This bump
  moved the protobuf runtime and its floor only; the generator stayed at
  `grpcio-tools` 1.83.1, so the generated output is byte-identical and
  `scripts/check-proto-drift.sh` passes. The distinction is between changes
  that regenerate artifacts and changes that merely share a group name.
...
- **The Dependabot batch lands on Saturday** (#529). All five ecosystems used
  a bare `weekly` interval with no `day`, which put the sweep on Sunday --
  observed at ~12:33 UTC across three consecutive weeks. Saturday delivers the
  pull requests a day ahead of project maintenance. Editing
  `.github/dependabot.yml` also forces an immediate re-check of every
  ecosystem, which is the only way to trigger a run from a commit: no public
  API exposes one, and the sole manual alternative is the per-ecosystem
  "Check for updates" button in the repository's Dependabot dashboard.
- `taiki-e/install-action` v2.87.0 -> v2.87.3 in the security workflow (#538).

### Documentation

- **The CodeQL Rust blind spot is recorded rather than worked around** (#522).
  Both memvid entry points `include!` a module generated by `build.rs`, and
  CodeQL never builds Rust, so `OUT_DIR` is unset and the gRPC handlers taking
  those types are extracted incompletely. Advanced setup does not fix it --
  CodeQL 2.26.4 offers Rust `build-mode: none` only, and rejects `manual`
  outright. `docs/SECURITY.md` now carries the gap and its compensating
  controls so the next attempt does not repeat the one that failed.
- **Base-image versions are no longer restated in prose** (#523). Every base
  is digest-pinned and bumped by Dependabot, which updates the `FROM` line and
  nothing else, so a version copied into a comment or a document rots
  silently. The `FROM` lines are the source of truth.
- **The specforge feature counts and phase status are no longer restated**
  (#527). `CLAUDE.md` claimed 124 features with 90 unverified long after the
  real numbers had moved to 175 and 0, and the phase table's Status column
  read "Complete" on every row with detail -- "Approved", "87.4/100" -- that
  nothing updates when a phase is re-run. Both are replaced by pointers to
  what actually answers the question: two `jq` commands against
  `feature_list.json`, and the artifact named in each phase's row.
